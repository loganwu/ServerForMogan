...
struct shm_queue
{
	...
	time_t fifo_times[SQ_MAX_READER_PROC_NUM]; // fifo creation timestamps
	uint64_t shm_key;
...
static int sq_set_sigparam(struct sq_head_t *sq, int sig_ele_num, int sig_proc_num)
{
...
struct shm_queue *sq_create(u64_t shm_key, int ele_size, int ele_count, int sig_ele_num, int sig_proc_num)
{
	...
	sq_set_sigparam(queue->head, sig_ele_num, sig_proc_num);
...
static int signal_process(struct shm_queue *sq, int sigidx)
{
	...
	if(sq->poll_fdset[sigidx]>0)
	{
		...
		{
			__sync_fetch_and_add(&sq->head->signr[sigidx], 1);
		}
